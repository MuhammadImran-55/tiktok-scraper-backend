...
async function scrapeHashtagVideos(hashtag) {
  ...
  const url = `https://www.tiktok.com/search?q=${encodeURIComponent(hashtag)}`;
  ...
  // Wait for at least one video card
  ...
  const videos = await page.evaluate(() => {
    ...
    return Array.from(nodes).map(node => ({
      ...
      username: node.querySelector('[data-e2e="search-card-user-unique-id"]')?.innerText || "Unknown",
      profileLink: node.querySelector('[data-e2e="search-card-user-link"]')?.href || null,
  ...
  // Convert likeCount to number and sort by likes descending
  const ranked = videos
    .map(v => ({ ...v, likes: parseCount(v.likeCount) }))
    .sort((a, b) => b.likes - a.likes);

  console.log("✅ Total videos scraped:", ranked.length);
  return ranked;
}
